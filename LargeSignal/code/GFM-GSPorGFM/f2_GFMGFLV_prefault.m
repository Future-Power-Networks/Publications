function dfdt = f2_GFMGFLV_prefault(x)
    ...
    elseif model == "rough" %ignore w2 & w1
        ...
        dfdt(2) = Kgfm*(Pref-Vgfm*Ug*sin(delta2)/(Xg+X2) + Xg/(Xg+X2)*I1d*Vgfm*cos(delta1-delta2) - Xg/(Xg+X2)*I1q*Vgfm*sin(delta1-delta2));
          ...
          dfdt(1) = k1p * ((X1+X2pg)*I1d+imag(Zg/Z2sum*Vgfm*exp((delta2-delta1)*1j))+imag(Z2/Z2sum*Ug*exp(delta1*-1j))+I1q*(R1+R2pg)); 
          dfdt(2) = Kgfm*(Pref +real(Zg/Z2sum*I1d*Vgfm*exp((delta1-delta2)*1j)) + real(Zg/Z2sum*I1q*Vgfm*exp((delta1-delta2+pi/2)*1j)) - real(Vgfm^2/Z2sum-Vgfm*Ug/Z2sum*exp((-delta2)*1j)) );
