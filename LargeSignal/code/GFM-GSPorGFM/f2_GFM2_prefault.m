function dfdt = f2_GFM2_prefault(x)
...
%         dfdt(2) = Kgfm2 * (Pref2 - Vgfm1*Vgfm2*X12/(X12*(1+R_ratio))^2*sin(delta2-delta1) - (Vgfm2^2-Vgfm1*Vgfm2*cos(delta2-delta1))*X12*R_ratio/(X12*(1+R_ratio))^2 - Vgfm2*Ug*sin(delta2)*X2g/(X2g*(1+R_ratio))^2 -(Vgfm2^2-Vgfm2*Ug*cos(delta2))*X2g*R_ratio/(X2g*(1+R_ratio))^2);
        dfdt(2) = Kgfm1 * (Pref1 - Vgfm1*Vgfm2/X12*sin(delta1-delta2) - Vgfm1*Ug/X1g*sin(delta1));
        dfdt(1) = Kgfm2 * (Pref2 - Vgfm1*Vgfm2/X12*sin(delta2-delta1) - Vgfm2*Ug/X2g*sin(delta2));
    else %resistance
        dfdt(2) = Kgfm1 * (Pref1 - real(Vgfm1^2/Z11-Vgfm1*Ug/Z1g*exp(-1*1j*delta1)- Vgfm1*Vgfm2/Z12*exp(1j*(delta2-delta1))));
        dfdt(1) = Kgfm2 * (Pref2 - real(Vgfm2^2/Z22-Vgfm2*Ug/Z2g*exp(-1*1j*delta2)- Vgfm1*Vgfm2/Z12*exp(1j*(delta1-delta2))));
    end
